...
namespace Phpify\Core\Exception;
...
class ErrorHandler
{
    ...
    public static function handleException(Throwable $exception): void
    {
        $isCli = PHP_SAPI === 'cli';
        $isJson = false;
        if (!$isCli && isset(\Phpify\Core\Application::$app)) {
            $isJson = \Phpify\Core\Application::$app->request->isJson();
        }

        $statusCode = 500;
        if ($exception instanceof \Phpify\Core\Http\ValidationException) {
            $statusCode = 422;
        }
